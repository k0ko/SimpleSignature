...
// ====== Флаг за предотвратяване на безкрайни цикли ======
...
// ====== OPTIMIZED: Функции за цвят без безкрайни цикли ======
...
// ====== OPTIMIZED: Събития за цветовите полета ======
...
// ====== Coloris глобална инициализация ======
if (window.Coloris) {
    Coloris({
        el: '.coloris-picker',
        theme: 'pill',
    ...
    document.querySelectorAll('.coloris-picker').forEach((picker) => {
        picker.addEventListener('input', () => {
            ...
            if (!hex || hex.value === picker.value) return;
...
// ====== Custom dropdowns ======
function makeCustomDropdown(source, type, options = {}) {
    ...
    const refresh = () => {
        ...
        switch (type) {
            case "scheme": {
                const active =
                    source.querySelector(".scheme.active") ||
                    source.querySelector(".scheme");

                if (active) {
                    trigger.appendChild(active.cloneNode(true));
                }
                break;
            }
            case "background": {
                ...
                swatch.style.background = active
                ...
                label.textContent = custom
                    ? "Custom"
                    : (active ? active.title : "Select color");
    ...
    source.addEventListener('click', (e) => {
        if (e.target.closest('.scheme, .dot, .background-custom')) setTimeout(() => {
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('.custom-dropdown')) {
        document.querySelectorAll('.custom-dropdown.is-open').forEach(x => x.classList.remove('is-open'));
    }
});
...
setBg = function (mode) {
    originalSetBg(mode);
    backgroundDropdownUI?.wrap.classList.toggle('is-disabled', mode !== 'color');
};
...
(function() {
    ...
    document.body.prepend(previews);
    ...
    document.body.style.paddingTop = previews.offsetHeight + "px";
    ...
    function updateExpandedHeight() {
        ...
        mode.style.height = expandedHeight + "px";
        document.body.style.paddingTop =
            (previews.offsetHeight - mode.offsetHeight + expandedHeight) + "px";
    }
    ...
    function animateHeight(from, to) {
        ...
        const headerHeight = previews.offsetHeight - mode.offsetHeight;
        ...
        function frame(now) {
            ...
            document.body.style.paddingTop = (headerHeight + h) + "px";

            if (t < 1) {
                raf = requestAnimationFrame(frame);
            } else {
                mode.style.height = to + 20 + "px";
                document.body.style.paddingTop = (headerHeight + to) + 20 + "px";
    ...
    window.addEventListener("scroll", () => {
        ...
        if (!previews.classList.contains("is-collapsed")) {
            ...
            animateHeight(expandedHeight, collapsedHeight);
        }

        scrollTimer = setTimeout(() => {
            animateHeight(collapsedHeight, expandedHeight);
        }, 2000);
    });
