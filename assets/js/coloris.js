...
((window, document, Math) => {
  ...
  function attachFields(selector) {
    ...
    addListener(document, 'click', selector, event => {
      console.log('ima promqna v pickera2');
      currentEl = event.target;
      oldColor = currentEl.value;

      const control = currentEl.closest(".color-control");

      if (control && picker.parentNode !== control) {
      ...
      colorAreaDims = {
        ...
        x: colorArea.getBoundingClientRect().left,
        y: colorArea.getBoundingClientRect().top
      };
  ...
  function setColorFromStr(str) {
    ...
    colorMarker.style.top = `${colorAreaDims.height * (1 - hsva.v / 100)}px`;
  ...
  function moveMarker(event) {
    let x = event.clientX - colorAreaDims.x;
    let y = event.clientY - colorAreaDims.y;
  ...
  function startMarkerMove(event) {
    ...
    document.addEventListener("pointerup", stopMarkerMove);
}

function stopMarkerMove() {
    document.removeEventListener("pointermove", moveMarker);
    document.removeEventListener("pointerup", stopMarkerMove);
}
  ...
  function init() {
    // Render the UI
    const isBackground = currentEl?.classList.contains("background-colors");
    ...
    picker.innerHTML =
      (isBackground
        ...
        : `<input id="clr-color-value" class="clr-color" type="text" value="" readonly aria-label="${settings.a11y.input}" onfocus="this.blur()" style="pointer-events: none; user-select: none; opacity: 0.7;">`
      ) +
    ...
    addListener(colorArea, "pointerdown", startMarkerMove);

    addListener(colorMarker, "pointerdown", startMarkerMove);
